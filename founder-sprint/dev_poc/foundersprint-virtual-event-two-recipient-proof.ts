...
(async () => {
  const now = Date.now();
  const batchName = `FS FINAL KST 0410 BATCH ${now}`;
  const companyName = `FS FINAL KST 0410 CO ${now}`;
  const eventTitle = `FS FINAL KST 0410 VIRTUAL EVENT ${now}`;
  const recipients = ["slit.amazing@gmail.com", "jgamer0914@gmail.com"];
  ...
  const recipientUsers = [] as Array<{ id: string; email: string }>;
  for (const email of recipients) {
    const user = await prisma.user.upsert({
      where: { email },
      create: { email, name: `Proof User ${email}`, status: "active", role: "founder" },
      update: { status: "active", name: `Proof User ${email}` },
  ...
  const batch = await prisma.batch.create({
    data: {
      name: batchName,
      description: "two-recipient live proof batch",
  ...
  const company = await prisma.company.create({
    data: { name: companyName, slug: `fs-final-kst-0410-${now}` },
    select: { id: true, name: true },
  });
  ...
  await prisma.userBatch.createMany({
    data: [
      ...
      ...recipientUsers.map((u) => ({ userId: u.id, batchId: batch.id, role: "founder" as const, status: "active" as const, joinedAt: new Date() })),
    ],
  });
  await prisma.companyMember.createMany({
    data: recipientUsers.map((u) => ({ companyId: company.id, userId: u.id, isCurrent: true })),
  });
  ...
  try {
    ...
    const result = {
      ...
      cleanup: {
        ...
        note: "Event intentionally left alive for two-recipient recipient-side verification. Clean up manually after confirmation.",
      },
    };

    console.log(JSON.stringify(result, null, 2));
    fs.writeFileSync("dev_poc/foundersprint-live-two-recipient-event-proof-result.json", JSON.stringify(result, null, 2));
    ...
    if (!attendeeEmails.includes("test-admin@example.com")) throw new Error("admin missing from attendee list");
    for (const email of recipients) {
      if (!attendeeEmails.includes(email)) throw new Error(`${email} missing from attendee list`);
    }
    if (attendeeEmails.length !== 3) throw new Error(`Unexpected attendee count: ${attendeeEmails.length}`);
