...
| 이름 | 최대 100자, 중복 허용 |
| 설명 | 선택, 최대 500자 |
| 동시 Active Batch | **1개만** (Active Batch 존재 시 새 Batch 생성 버튼 비활성화) |
...
| Archived 열람 | 해당 기수 참여자 + Admin만 가능 |
...
Request 상태:
  ...
  approved → cancelled (Admin만 취소 가능)
```
...
| Calendar Invite | 참석자(Founder, Mentor/Admin) 전원에게 발송 |
...
## 12. Notification 규칙 (MVP)

### 12.1 필수 알림

| 트리거 | 발송 대상 | 방식 |
|--------|----------|------|
| 오피스아워 승인 | Host + Founder | Google Calendar Invite |

### 12.2 Optional 알림 (시간/예산 여유 시)

| 트리거 | 수신자 | 방식 |
|--------|--------|------|
| 질문 등록 | Admin + 전체 Mentor | 이메일 |
| 요약 등록 | 질문 작성자 (Founder) | 이메일 |
...
### 14.2 Office Hour Request State
